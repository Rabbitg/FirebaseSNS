...
import kotlinx.android.synthetic.main.card_post.view.imageView
...
class WriteActivity : AppCompatActivity() {
    ...
    /**
     * 배경 리스트 데이터
     * res/drawable 디렉토리에 있는 배경 임지ㅣ를 uri 주소로 사용한다.
     * uri 주소로 사용하면 추후 웹에 있는 이미지 URL 도 바로 사용이 가능하다.
     * */
    val bgList = mutableListOf(
        "android.resource://com.example.rabbit/drawable/default_bg"
        ,"android.resource://com.example.rabbit/drawable/bg2"
        ,"android.resource://com.example.rabbit/drawable/bg3"
        ,"android.resource://com.example.rabbit/drawable/bg4"
        ,"android.resource://com.example.rabbit/drawable/bg5"
        ,"android.resource://com.example.rabbit/drawable/bg6"
        ,"android.resource://com.example.rabbit/drawable/bg7"
        ,"android.resource://com.example.rabbit/drawable/bg8"
        ,"android.resource://com.example.rabbit/drawable/bg9"
    )
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sendButton.setOnClickListener {
            ...
            if (mode == "post") {
                // Post 객체 생성
                val post = Post()
                // Firebase 의 Posts 참조에서 객체를 저장하기 위한 새로운 키를 생성하고 참조를 newRef에 저장
                ...
                commnet.writeTime = ServerValue.TIMESTAMP
                commnet.bgUri = bgList[currentBgPosition]
                // 메세지는 input EditText 의 텍스트 내용을 할당
                commnet.message = input.text.toString()
                // 글쓴 사람의 ID 는 디바이스의 아이디로 할당
                commnet.writerId = getMyId()
                // 글의 ID는 새로 생성된 파이어베이스 참조의 키로 할당
                commnet.commentId = newRef.key!!
                // 댓글이 속한 글의 ID
                commnet.postId = postId
                newRef.setValue(commnet)
                // 저장성공 토스트 알림을 보여주고 Activity 종료
                Toast.makeText(applicationContext,"공유되었습니다.",Toast.LENGTH_SHORT).show()
                finish()
            }
    ...
    inner class MyAdapter : RecyclerView.Adapter<MyViewHolder>(){
        ...
        override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
            ...
            holder.itemView.setOnClickListener{
                ...
                Toast.makeText(applicationContext,"클릭테스트"+position,Toast.LENGTH_LONG).show()
                Picasso.get()
                    .load(Uri.parse(bgList[position]))
                    .fit()
                    .centerCrop()
